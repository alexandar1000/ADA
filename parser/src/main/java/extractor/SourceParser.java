	...
	public Set<SourceFile> parseSource() {
		...
		try {
			...
			cu.findAll(ClassOrInterfaceDeclaration.class).forEach(cl -> {
				...
				Map<String, String> staticVaribales = new HashMap<String, String>();
				cl.resolve().getAllStaticFields().forEach(x -> {
					staticVaribales.put(x.getName(), x.getType().describe());
				});

				// all the parents class interfaces,
				...
				//System.out.println(parentClasName);
				...
				cl.findAll(MethodDeclaration.class).forEach(n -> {
					...
					Map<String, String> usedVaribales = new HashMap<String, String>();
					...
					n.findAll(VariableDeclarator.class).forEach(V->{
						...
						usedVaribales.put(v.getNameAsString(), v.getTypeAsString());
					});

					SourceMethod sm = new SourceMethod(methodName, returnType, accessModifiers, parameters,
							methodCallExpression, usedVaribales);
					methods.add(sm);
				});
				sourceClasses.add(new SourceFile(className, parentClassName,implementedInterface,staticVaribales, methods));
