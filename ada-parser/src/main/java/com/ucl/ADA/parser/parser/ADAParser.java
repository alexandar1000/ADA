...
public class ADAParser {
    ...
    public Set<ADAClass> getParsedSourceFile(String sourceRoot) {
        ...
        List<String> filePaths = SourceFileCollector.getJavaFilesFromSourceDirectory(new File(sourceRoot));
        String[] allSrcDirectories = getAllSrcDirectories(new File(sourceRoot));
        filePaths.forEach(file -> {
            ASTParser parser = buildASTParser(sourceRoot, allSrcDirectories);
...
/*
        ...
        String[] allSrcDirectories = getAllSrcDirectories(new File(src_dir));
        for (int i = 0, filePathsSize = filePaths.size(); i < filePathsSize; i++) {
            ...
            ASTParser parser = buildASTParser(src_dir, allSrcDirectories);
    ...
    private ASTParser buildASTParser(String sourceRoot, String[] allSrcDirectories) {
        String[] encoding = new String[allSrcDirectories.length];
        ...
        String[] sources = allSrcDirectories;
        String[] classpath = {};
    ...
    private List<CompilationUnit> getCompilationUnits(Map<String, String> sourceCodes, String sourceRoot) {
        ...
        for (String path : sourceCodes.keySet()) {
            try {
                ...
                System.err.println("Exception occured in " + path);
            }
        }
